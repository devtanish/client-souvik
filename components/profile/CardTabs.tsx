...
export const MetricCard = ({ title, value, trend, type = 'number', description, lastUpdate, onClick, isActive }: { title: string; value: number; trend: number; type?: 'number' | 'money'; description: string; lastUpdate: string; onClick?: () => void; isActive?: boolean }) => {
  ...
  return (
    ...
    >
      ...
      <div className="text-xl sm:text-4xl font-serif text-gray-900 mb-2 md:mb-3">
        {formatValue(value, type)}
      </div>
      
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-gray-900 text-xs sm:text-sm font-medium">
          <TrendingUp className="w-4 h-4" />
          <span>{description}</span>
        </div>
        <p className="text-gray-500 text-[10px] sm:text-xs">{lastUpdate}</p>
      </div>
    </div>
  );
};